...
if not exist "%VENDOR%\node.exe" (
  echo Keep the whole Tube Stash folder together. vendor\win-x64\node.exe is missing.
  pause
  exit /b 1
)
if not exist "%CD%\server\index.js" (
  echo Keep the whole Tube Stash folder together. server\index.js is missing.
  pause
  exit /b 1
)

echo Starting Tube Stash at http://127.0.0.1:47841
